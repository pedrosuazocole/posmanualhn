...
async function _textoArticulos(turnoId, turno, ventas) {
  ...
  for (const v of arr) {
    try {
      ...
      for (const item of items) {
        const key = item.producto_nombre || item.nombre || 'Artículo';
        if (!articulos[key]) articulos[key] = { cant:0, total:0, precio: parseFloat(item.precio_unit||item.precio||0) };
  ...
  const lines = [
    sep32,
    empresa.toUpperCase().padStart(Math.ceil((32+empresa.length)/2)).padEnd(32),
    'ARTICULOS VENDIDOS'.padStart(Math.ceil((32+18)/2)).padEnd(32),
    pad('Turno:',turno.turno_letra||'A',32),
    pad('Cajero:',turno.usuario_nombre||'—',32),
    sep16,
    pad('PRODUCTO','CANT   TOTAL',32),
    sep16,
  ];

  const sorted = Object.entries(articulos).sort((a,b)=>b[1].total-a[1].total);
  for (const [nombre, data] of sorted) {
    const nom = nombre.length>18 ? nombre.substring(0,18) : nombre;
    const val = data.cant+'u  L.'+data.total.toFixed(2);
    lines.push(pad(nom, val, 32));
  }

  if (!sorted.length) lines.push('  Sin articulos registrados');
  lines.push(sep16);
  lines.push(pad('TOTAL GENERAL:', 'L.'+totVentas.toFixed(2), 32));
  lines.push(pad('N° Facturas:', String(arr.length), 32));
  lines.push(sep32);
  lines.push('Powered by MetricPOS'.padStart(26));
  lines.push(sep32);
